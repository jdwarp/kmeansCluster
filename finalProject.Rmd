---
title: "Final Project Cluster Analysis MGMT 6770"
author: "Jaime Duran"
date: "12/10/2019"
output: 
  html_document:
...
1. Summary of Methodology
  ...
  We begin by loading in the data set of sales transactions at a large store.The data measres different categories such as quantities and price points of each transaction, which means that the data needs to be reworked before it can be compared (analogous to finding a common denomenator in simple algebra). We had to "normalize it" which is a way of scaling values in each category between 1 and -1 using the scale function. We do this to control for the influence of different measuring units, and it lets us analyze and compare within variable variance. 
  Once the dataframe was normalized we could proceede and form clusters. We tried to find the optimal way of getting a cluster 100 percent full of confirmed fraudulent data entries. As a class we saw in "withinclusterplot"" that the number of centers or clusters was optimal at 6 centers. 
  Afterward we feed the machine data that needs to be inspected because its status is unknown. This is where the 'unsupervised' aspect comes in. We can infer that transactions which are close to this cluster share qualities to fraud and are likely to be fraud. However, we don't know if they are fraud just because they share a striking resemblance to fraud statistically speaking.

2. Identifiable patterns
  Sampling the first 50 and the first 100 of the inputs that have attributes similar to the instances of confirmed fraud.The first 50 suspected fraudulent samples were attributable to 14 individual employee ID's. When I doubled the sample size by changing nstart to 100, the number of ID's scratched together was 22 unique values. 9 out of the 14 employees identified in the class example showed up in my sample when nstart was changed to 100. These employees are: v2931 v2959 v2966 v4870 v3901 v5521 v5086 v242 & v4840. 
The boxplot and summary of fraudulent data suggest that most instances of fraud are disproportional in one or another way. Either the quantity of goods is too large in one transaction or the price of a transaction is large and going to one employee. I would feel uneasy if I were an employee on this list!
